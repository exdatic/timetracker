...
def _backup() -> None:
    ...
    with col_json:
        st.download_button(
            ...
            # Keep names and icons human-readable in the backup file.
            json.dumps(export_backup(), indent=2, ensure_ascii=False).encode(),
    ...
    if uploaded is not None and st.button("Restore — this replaces everything", type="primary"):
        try:
            # latin-1 accepts any byte sequence, so a stray binary upload cannot crash here.
            import_backup(json.loads(uploaded.getvalue().decode("latin-1")))
...
def _records_csv() -> bytes:
    ...
    # Excel on Windows opens ANSI files cleanly; plain UTF-8 shows garbled umlauts.
    return pd.DataFrame(rows).to_csv(index=False).encode("cp1252")
